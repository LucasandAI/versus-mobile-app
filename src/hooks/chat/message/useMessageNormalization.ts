  ...
  const normalizeMessage = (message: any): ChatMessage => {
    ...
    if (
      ...
      return {
        ...
        isUserMessage: String(message.sender.id) === String(currentUserId)
      };
    }
    ...
    if (message.message !== undefined && message.sender_id !== undefined) {
      ...
      return {
        ...
        timestamp: message.timestamp || message.created_at || new Date().toISOString(),
        isSupport: false,
        isUserMessage: String(message.sender_id) === String(currentUserId)
      };
    }
    ...
    return {
      ...
      timestamp: message.timestamp || message.created_at || new Date().toISOString(),
      isSupport: false,
      isUserMessage: String(message.sender_id || message.sender?.id) === String(currentUserId)
    };
  };
